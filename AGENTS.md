...
- The live web calibration defaults to `calibrations/quest_teleop_frame.json`; named profiles are saved as `calibrations/<profile>.json`; run `scripts/run_quest_tracker_hub.sh --profile <profile>` first, then `scripts/run_libero_teleop.sh --profile <profile> --input-source target`.
