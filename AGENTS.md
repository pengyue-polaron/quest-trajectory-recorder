...
- The live web calibration defaults to `calibrations/quest_teleop_frame.json`; named profiles are saved as `calibrations/<profile>.json` and passed to LIBERO with `scripts/run_libero_teleop.sh --profile <profile>`.
